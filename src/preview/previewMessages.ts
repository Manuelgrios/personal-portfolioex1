...
const previewChannelPattern = /^[a-zA-Z0-9_-]{16,96}$/;

export function isValidPreviewChannelId(value: unknown): value is string {
  return typeof value === "string" && previewChannelPattern.test(value);
}
...
export function isFolioDevTemplatePreviewMessage(
  ...
  if (
    ...
    !isValidPreviewChannelId(expectedChannelId) ||
    !isRecord(value.data)
  ) {
    return false;
  }
